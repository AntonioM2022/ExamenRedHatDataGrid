...
DELIMITER $$
CREATE PROCEDURE `sp_insert_film`(IN `p_title` VARCHAR(128), IN `p_description` TEXT,
                                  IN `p_release_year` YEAR(4), IN `p_language_id` TINYINT(3),
                                  IN `p_original_language_id` TINYINT(3),
                                  IN `p_rental_duration` TINYINT(3),
                                  IN `p_rental_rate` DECIMAL(4, 2), IN `p_length` SMALLINT(5),
